...
attendanceSchema.pre('save', async function(next) {
  try {
    ...
    // Get work hours from system settings with error handling
    let workHours;
    try {
      workHours = await SystemSettings.getWorkHours();
    } catch (error) {
      console.error('Error getting work hours from SystemSettings:', error);
      // Use default values if SystemSettings fails
      workHours = {
        checkInTime: '09:00',
        checkOutTime: '18:00',
        workingHours: 8,
        minimumWorkHours: 6,
        lateThreshold: 0.5 // 30 minutes in hours
      };
    }
    ...
    const lateThresholdMinutes = workHours.lateThreshold * 60; // Convert hours back to minutes
